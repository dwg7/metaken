...
- [x] Resolve code-list values into human-readable labels (topicCategory_label/hierarchyLevel_label/dateType_label/contactRole_label columns, plus a report section -- see § "code-list resolution + survey-type keyword fix")
...
**2026-07-17 — code-list resolution + survey-type keyword fix**

Picked up two of the three remaining §15 items ("small polish, then decide
whether to keep going or stop" per user framing) -- the third (full-scale
reconciliation against 公共測量実施情報) was left as-is, since the existing
sample already answered its direction and a full reconciliation needs a
rate-conscious multi-session effort against a citizen-facing tool, not a
quick pass.

**1. `SURVEY_TYPE_MAP_PRODUCT` keyword fix.** Added `航空写真` (the F0532
false-negative diagnosed in the previous log entry: title
"高槻市デジタル航空写真撮影業務" used 航空写真, not the only aerial-photo
synonym the list had, 空中写真). Deliberately did **not** add the other terms
this log's own previous entry floated (数値地形図データの作成, 既成図数値化,
修正測量, 写真地図作成) -- 写真地図作成 and 数値地形図データの作成 already
match via existing substrings (写真地図, 数値地形図), and the other two were
only ever seen as *official* 測量種別 category labels for I0392/B0459/B0131,
not as confirmed text actually present in those records' title/abstract;
adding them without the underlying text would be guessing at a fix rather
than repeating the same kind of concrete diagnosis as 航空写真. Effect on the
full R05-R07 dataset: 測量メイン 7,549→7,443, 地図メイン 2,256→2,362 (106
records reclassified) -- small, and in the direction the sample validation
said the split was biased (undercounting 地図メイン), consistent with that
finding rather than a new one.

**2. Code-list label resolution.** topicCategory/hierarchyLevel/dateType/role
were stored as raw JMP2.0 `codeListValue` numbers (e.g. `013`) with no label
anywhere in the pipeline -- not even in the report prose, which had been
citing "topicCategory is 83% one value" from an ad-hoc one-off check, not a
committed, reproducible figure. Resolved against JMP2.0's own spec
(https://www.gsi.go.jp/common/000259949.pdf) -- fetched the PDF, ran
`pdftotext -layout`, and read both the Appendix C XML Schema enumeration
order and the §5.1/5.2/5.7/5.9 code tables directly, rather than assuming
the codes follow plain ISO 19115 codelist order (they do, in this case, but
that was verified against GSI's own document, not assumed from memory).
Added `TOPIC_CATEGORY_CODES`/`HIERARCHY_LEVEL_CODES`/`DATE_TYPE_CODES`/
`ROLE_CODES` lookup tables and a `resolve_codelist()` helper in `parse.py`;
new `*_label` columns (`topicCategory_label` etc.) sit alongside the raw
code columns in the CSV, keeping the existing raw-code columns as the
empirical record (same rationale as the original raw-CSV-commit decision).
New "Code List Distributions" section in both reports, using
`compute_codelist_distribution()` in `report.py` (counts blank separately
from resolved labels, since e.g. contactRole being blank in 95% of records
is a different fact from contactRole having an unresolved code).

Confirms rather than overturns the prose estimate: topicCategory 82%
`位置 (location)` (of 11,059; 9,159/9,724 non-blank = 94%), hierarchyLevel 85%
`データ集合 (dataset)`, dateType 88% `作成日 (creation)`, role 95% blank
(matches the original pilot's "contact organisation: 96% of files are
missing it" finding, from a different but related field).
